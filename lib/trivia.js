...
import { eat, eatMatch, mapProductions, resolveLanguage } from './grammar.js';
...
export const triviaEnhancer = (
  ...
  const resultGrammar = mapProductions((production) => {
    return function* (props) {
      ...
      const { ctx, s, covered, flags } = props;
        ...
        while (!co.done) {
          ...
          switch (verb) {
            ...
            case 'guard': {
              ...
              if (matcher.type === EmbeddedRegex || typeof matcher === 'string') {
              ...
              let name = getCooked(matcher.value.properties.refMatcher?.node.properties.name?.node);
              ...
              let isCoverBoundary = isCover || (isNode && !covered);
              if (
                matcher &&
                ...
                if (triviaIsAllowed(s)) {
                  ...
                  if (result && !result.isNull) {
                    ...
                    result = result.merge(trivialResult);
                  }
                  returnValue = result;
                } else {
                  returnValue = yield instr;
            ...
            if (triviaIsAllowed(s)) {
              ...
              return buildCall(co.value.verb, m`<${buildIdentifier(Wrapper_)} />`, [
                ...
                buildEmbeddedInstruction(
                  buildCall('eat', co.value.arguments[0], co.value.arguments[1]),
                ),
              ]);
            }
          }
