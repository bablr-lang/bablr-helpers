...
export function resolveLanguage(languages, path) {
  ...
  let path_ = Array.isArray(path)
    ? path
    : Array.prototype.flatMap.call(reifyExpression(path.value).bindingMatchers, (m) => m.segments);

  for (let { name, type } of arrayValues(path_)) {
    if (name) {
...
export const startSpan = (name, guard, props = freezeRecord({})) => {
  if (!isDeepRecord(props)) throw new Error();
  return __buildCall('startSpan', name, guard, false, o(props));
};

export const startSubspan = (name, guard, props = freezeRecord({})) => {
  if (!isDeepRecord(props)) throw new Error();
