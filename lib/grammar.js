...
import { buildBindingMatcher } from './builders.js';
...
function* __wrapGenerator(generator, relativeMatcher) {
  ...
  if (!relativeBinding || !relativeBinding.segments.length) {
  ...
  while (!(step = generator.next(returnValue)).done) {
    ...
    if (isMatcher) {
      ...
      let newMatcher = Path.set(
        ...
        'bindingMatcher',
        buildBindingMatcher(
          binding ? [...relativeBinding.segments, ...binding.segments] : relativeBinding.segments,
        ),
      );
...
export function resolveLanguage(languages, path) {
  ...
  let path_ = Array.isArray(path)
    ? path
    : reifyExpression(get(['bindingMatcher'], path.value))?.segments ?? [];
