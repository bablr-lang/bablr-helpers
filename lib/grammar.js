...
import { buildBindingMatchers, buildPropertyMatcher } from './builders.js';
...
function* __wrapGenerator(generator, relativeMatcher) {
  ...
  while (!(step = generator.next(returnValue)).done) {
    ...
    if (isMatcher) {
      ...
      if ((nodeMatcher.flags.token && !nodeMatcher.type) || !relativeBindings.length) {
      ...
      let newMatcher = buildPropertyMatcher(
        ...
        buildBindingExpression([...relativeBindings, ...bindingMatchers]),
...
export function resolveLanguage(languages, path) {
  ...
  let path_ = Array.isArray(path)
    ? path
    : reifyExpression(get(['bindingMatchers'], path.value))?.segments ?? [];
