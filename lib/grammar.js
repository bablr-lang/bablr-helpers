...
import { buildBindingMatcher, buildIdentifierPath } from './builders.js';
...
function* __wrapGenerator(generator, relativeMatcher) {
  let relativeBinding = reifyExpression(Path.get(relativeMatcher.value, 'bindingMatcher'));

  if (!relativeBinding || !relativeBinding.languagePath.length) {
  ...
  while (!(step = generator.next()).done) {
    ...
    let isMatcher =
      matchVerbs.has(verb) &&
      args[0].type === Matcher &&
      args[0].value.type?.description === 'PropertyMatcher';

    if (isMatcher) {
      let { 0: matcher, 1: props, 2: options } = args;
      // TODO merge these
      let binding = reifyExpression(Path.get(matcher.value, 'bindingMatcher'));

      let newMatcher = Path.set(
        matcher.value,
        'bindingMatcher',
        buildBindingMatcher(
          buildIdentifierPath(
            binding
              ? [...relativeBinding.languagePath, ...binding.languagePath]
              : relativeBinding.languagePath,
          ),
        ),
      );
...
export function resolveLanguage(state, path) {
  let { languages } = state;
  ...
  let path_ = Array.isArray(path)
    ? path
    : reifyExpression(get(['bindingMatcher', 'languagePath'], path.value)) ?? [];
...
export function* exec(getState, matcher, value, getGrammar = (g) => g) {
  ...
  let language = resolveLanguage(state, matcher);
  ...
  let generator = grammar_.prototype[nodeMatcher.type](args);

  return wrapGenerator(generator, matcher);
}
