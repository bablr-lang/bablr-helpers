...
export const buildBindingMatcher = (languagePath) => {
  let expressions = [];
  const gap = buildFilledGapFunction(expressions);
...
export const buildReferenceFlags = (flags = t.referenceFlags) => {
  ...
  return treeFromStream(
    [
      ...
      gap(intrinsic ? buildToken('Punctuator', '*') : buildNullNode()),
      t.nodeClose(),
    ],
    { expressions },
  );
...
export const buildNodeFlags = (flags = t.nodeFlags) => {
  ...
  return treeFromStream(
    [
      ...
      gap(fragment && !cover ? buildToken('Punctuator', '_') : buildNullNode()),
      t.nodeClose(),
    ],
    { expressions },
  );
