...
export const buildPropertyMatcher = (refMatcher, bindingMatchers, nodeMatcher) => {
  const expressions = [];
  const gap = buildFilledGapFunction(expressions);
  ...
  return treeFromStream(
    (function* () {
      ...
      yield gap(buildBoundNodeMatcher(bindingMatchers || [], nodeMatcher));
      yield t.nodeClose();
    })(),
    { expressions },
  );
...
export const buildBasicNodeMatcher = (open) => {
  ...
  return treeFromStream(
    [t.nodeOpen(t.nodeFlags, 'TreeNodeMatcher'), t.ref`open`, gap(open), t.nodeClose()],
    { expressions },
  );
};
