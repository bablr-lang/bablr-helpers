...
export const buildPropertyMatcher = (refMatcher, valueMatcher, nodeMatcher) => {
  if (nodeMatcher !== undefined) throw new Error('bad property matcher');
...
export const buildNodeFlags = (flags = t.nodeFlags) => {
  ...
  let flags_ = { token, hasGap, fragment, cover };
...
export const buildSpamMatcher = (type = null, value = null, attributes = null) => {
  return buildTreeNodeMatcherOpen(buildNodeFlags(t.nodeFlags), null, type, value, attributes);
};

export const buildTreeNodeMatcherOpen = (flags, type, literalValue, attributes = null) => {
  const expressions = [];
  const gap = buildFilledGapFunction(expressions);
...
export const buildToken = (type, value, attributes = {}) => {
  return treeFromStream([
    t.nodeOpen(t.tokenFlags, type, null, attributes),
    t.lit(value),
    t.nodeClose(),
  ]);
};
...
export const buildArrayElements = (values) => {
  ...
  return treeFromStream(
    (function* () {
      yield t.nodeOpen(t.fragmentFlags);
...
export const buildObjectProperties = (properties) => {
  ...
  return treeFromStream(
    concat(
      [t.doctype({ 'bablr-lang': l.Instruction }), t.nodeOpen(t.nodeFlags)],
...
export const buildElements = (elements) => {
  ...
  return treeFromStream(
    concat(
      [t.doctype({ 'bablr-lang': l.Instruction }), t.nodeOpen(t.fragmentFlags)],
...
export const buildCharacterClass = (elements, negate = false) => {
  ...
  return treeFromStream(
    (function* () {
      ...
      yield* interpolateFragment(elements, t.ref`elements[]`, expressions), yield t.ref`closeToken`;
      yield gap(buildToken(null, ']'));
      yield t.nodeClose();
    })(),
    { expressions },
