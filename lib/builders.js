...
import { arrayValues, map } from '@bablr/agast-helpers/iterable';
...
import { parseDigits, parseObject, parseTag } from '@bablr/agast-helpers/parsers';
...
export const writeTag = (tag) => {
  ...
  switch (tag_.type) {
    ...
    case HashTag: {
      let { hash } = tag_.value;
      return writeHashTag(hash);
    }

    case EscapeTag: {
      let { value, cookedValue } = tag_.value;
      return writeEscapeTag(value, cookedValue);
    }

    case MuxerTag: {
      let { processPath, stream } = tag_.value;
      return writeMuxerTag(processPath, stream);
    }
...
export function* writeMuxerTag(processPath, stream) {
  yield '<MuxerTag>';
  yield 'openToken:';
  yield '<* "<" />';
...
export function* writeReferenceFlags(flags = t.parseReferenceFlags()) {
...
export function* writeNodeFlags(flags = t.parseNodeFlags()) {
  const { token = null, hasGap = null } = flags;

  let attributes = freezeRecord({ token, hasGap });
...
export function* writeTreeNodeMatcherOpen(matcher) {
  ...
  let attrsObj = parseObject(attributes);
...
export function* writeToken(name, value, attributes = freezeRecord({})) {
  if (!isDeepRecord(attributes)) throw new Error();
  if (value) {
    let attrsEmpty = !Object.keys(attributes);
    let attrsPart = attrsEmpty ? '' : ` ${printObject(attributes)}`;

    yield `<*${name ?? ''} ${printString(value)}${attrsPart} />`;
  }
}

export const writePunctuator = (value, attributes = freezeRecord({})) => {
...
export function* writeOpenNodeTag(open) {
  ...
  yield* when(literalValue, ['#:', "<* ' ' />"]);
  yield 'literalValue:';
  yield* literalValue ? writeTag(printTag(literalValue)) : ['null '];
...
export function* writeEscapeTag(value, cookedValue = null) {
  yield '<EscapeTag>';
  ...
  if (unquoted) {
    ...
    for (const piece of pieces) {
      if (/[\\`]/y.test(piece)) {
        ...
        yield `@${printString(chr)}@@${printString(`\\${chr}`)}`;
      } else {
        lit += piece;
      }
    }
...
export function* writeUnsignedInteger(value, base = 10) {
  const digits = value.toString(base).split('');

  yield '<UnsignedInteger>';
  for (let digit of digits) {
    yield 'digits[]:';
    yield* writeDigit(digit);
  }
...
export function* writeNumber(value) {
  if (!Number.isFinite(value)) {
    yield* writeInfinity(value);
  } else {
    let p = buildParser(String(value));
    let { str } = p;
    let chr = str[p.idx];
    ...
    if (chr == '0') {
      chr = str[++p.idx];
      yield 'wholePart:';
      yield* writeUnsignedInteger(0);
    } else {
      let value = parseDigits(p);
      chr = str[p.idx];
      yield 'wholePart:';
      yield* writeUnsignedInteger(value);
    }

    if (chr === '.') {
      chr = str[++p.idx];

      yield 'decimalSeparatorToken:';
      yield '<* "." />';
      let value = parseDigits(p);
      chr = str[p.idx];
      yield 'decimalPart:';
...
export function* writeNullTag() {
  ...
  yield 'sigilToken:';
  yield '<* "null" />';
  yield '#separatorToken:';
  yield '<* " " />';
  yield '</>';
}

export function* writeHashTag(hash) {
  yield '<HashTag>';
  yield 'openToken:';
  yield '<* "##" />';
  yield 'hash:';
  yield `<* ${printString(hash)} />`;
  yield 'closeToken:';
  yield '<* "##" />';
  yield '</>';
}

export function* writeRefsSums(refsSums) {
  yield 'openToken:';
  yield '<* "{" />';
  yield '#:';
  yield '<* " " />';
  ...
  for (let i = 0; i < refsSums.length; i += 3) {
    yield 'refsSums[]$';
    yield* writeRefSums(refsSums[i], refsSums[i + 1], refsSums[i + 2]);
...
export function* writeRefSums(ref, count, isArray) {
  yield 'ref:';
  yield* writeReferenceTag(parseTag(ref));
  yield '#:';
  yield '<* " " />';
  yield 'count:';
  yield ':JSON:';
  yield* writeNumber(count);
  yield '#separatorToken:';
  yield '<* "," />';
  yield '#:';
  yield '<* " " />';
  yield 'isArray:';
  yield ':JSON:';
  yield* writeNumber(Number(isArray));
}

export function* writeSumsTag(sums) {
...
export function* writeRegexFlags(flags = defaultRegexFlags) {
  ...
  let flags_ = { ...flags, sticky: false };

  for (let name of objectKeys(flags_)) {
    yield name + 'Token:';

    yield* flags_[name] ? writeToken(null, flagChrs[name]) : ['null '];
  }
  yield '</>';
}
...
export function* writeElement(element) {
  switch (typeof element) {
    case 'string': {
      ...
      if (escaped[chr]) {
        yield `<*Character>`;
        yield `@${printString(chr)}@@${printString(`\\${escaped[chr]}`)}`;
        yield '</>';
      } else if (chr < '\u001A') {
        let escapeCode = chr.charCodeAt(0).toString(16).padStart(4, '0');
        let escaped_ = String.fromCodePoint(escapeCode);
        yield '<*Character>';
        yield `@${escaped_}@@'\\\\u${escapeCode}`;
        yield '</>';
      } else {
...
export function* writeCharacterClassElement(element) {
  switch (typeof element) {
    case 'string':
      ...
      if (classEscaped[chr]) {
        yield '<*Character>';
        yield `@${printString(chr)}@@${printString(`\\${classEscaped[chr]}`)}`;
        yield '</>';
      } else if (chr < '\u001A') {
        let escapeCode = chr.charCodeAt(0).toString(16).padStart(4, '0');
        ...
        yield '<*Character>';
        yield `@${printString(escaped_)}@@'\\\\u${escapeCode}'`;
        yield '</>';
      } else {
        yield* writeToken('Character', chr);
      }
    ...
    case 'symbol':
      switch (element.description) {
        ...
        case 'D':
          yield '<CharacterClassRange { negate: true }>';
          yield 'escapeChr:';
          yield String.raw`<* "\\" />`;
          yield 'value:';
          yield '<*Keyword "D" />';
...
export function* writeCharacterClassElements(elements, negate = false) {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    } else if (elements[i] === '-' && (i === 0 || i === elements.length - 1)) {
      yield* writeToken('Character', '-');
    } else {
...
export function* writeQuantifier(el, min = 0, max = Infinity, greedy = true) {
  ...
  if (chr) {
    yield `<Quantifier { greedy: ${greedy}, max: ${max}, min: ${min} }>`;
    yield 'element+:';
    yield* el;
    yield 'sigilToken:';
    ...
    yield 'min:';
    yield* writeUnsignedInteger(min);

    if (max !== Infinity) {
      yield 'separatorToken:';
      yield* writeToken(null, ',');
      yield 'max:';
      yield* writeUnsignedInteger(max);
    }
    yield 'closeToken:';
    yield* writeToken(null, '}');
    yield '</>';
...
export function writeBABLRExpressionDeep(expr) {
  ...
  switch (typeof expr) {
    ...
    case 'object': {
      ...
      if (isArray(expr)) {
        return writeArray(writeArrayElements(arrayMap(expr, (e) => writeBABLRExpressionDeep(e))));
      } else {
        switch (expr.type) {
          case Object_:
            return __writeObject(
...
function* __writeCSTML(tags, options) {
  ...
  for (;;) {
    ...
    let tag = step.value;
    let tagType = t.parseTagType(tag);
