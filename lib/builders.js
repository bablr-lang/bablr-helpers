...
export function* writeNodeFlags(flags = parseNodeFlags()) {
  const { array = false, object = false, token = false } = flags;

  let attributes = freezeRecord({ array, object, token });
...
export function* writeOpenNodeTag(open) {
  ...
  if (flags.token && (flags.object || flags.array)) throw new Error();
  ...
  if (flags.object) {
    yield 'closeObjectFlagToken:';
    yield writePunctuator('}');
  } else if (flags.array) {
    yield 'closeArrayFlagToken:';
    yield writePunctuator(']');
  }
  yield* when(literalValue, ['#:', "<* ' ' />"]);
  yield 'literalValue:';
...
export function* writeSumsTag(sums) {
  ...
  yield* interpolateFragment(
    isArray(sums) ? writeArrayElements(map(writeJSExpressionDeep, arrayValues(sums))) : sums,
  );
